...
public:
	...
	static TraCITestClient* getInstance();
	 
    bool run(std::string fileName, int port, std::string host = "localhost");
    void V2X_run();	//utilize this thread for getting the subscribed data only
    ...
    Position V2X_getPosition(std::string vehicleID);
    ...
    int V2X_getSimulationTime(std::string vehicleID);//return the simulation time in seconds
	...
	void V2X_simulationEngine(std::string);
